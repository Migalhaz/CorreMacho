    ...
    public class PoolManager : Singleton<PoolManager>
    {
        ...
        void SetupPoolManager()
        {
            ...
            foreach (Pool pool in m_pools)
            {
                ...
                {
                    MigalhazHelper.LogError("Empty pool found!", this);
                    continue;
                }
                if (startedPools.Exists(x => x.m_PoolData == pool.m_PoolData))
                {
                    MigalhazHelper.LogError("Duplicate pool found!", this);
        ...
        public Pool GetPool(PoolData _poolData, bool _debugErrors = true)
        {
            bool PoolCheck(List<Pool> availablePools)
            {
                ...
                {
                    if (_debugErrors) MigalhazHelper.LogError("No pool found!");
                    return false;
                }
                ...
                {
                    if (_debugErrors) MigalhazHelper.LogError("More than 1 pool found!");
                    return false;
                }
                return true;       
            }
    ...
    public class Pool 
    {
        ...
        public T PullObject<T>(System.Action<GameObject> action = null) where T : Component
        {
            ...
            if (!m_components.ContainsKey(gameObject))
            {
                ...
                MigalhazHelper.LogError($"{typeof(T)} component not found!");
                return null;
            }
